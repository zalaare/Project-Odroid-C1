...
post_install() {
  sed -i "s|tty1|ttys0|g" $ROOT/$BUILD/initramfs/init
  chmod 0755 $ROOT/$BUILD/initramfs/init

  # platform_init
  if [ -f "$ROOT/.libreelec/projects/$PROJECT/initramfs/platform_init" ]; then
    install -m 0755 $ROOT/.librelec/projects/$PROJECT/initramfs/platform_init $ROOT/$BUILD/initramfs/platform_init
  elif [ -f "$ROOT/.librenelec/initramfs/platform_init" ]; then
    install -m 0755 $ROOT/.librenelec/initramfs/platform_init $ROOT/$BUILD/initramfs/platform_init
